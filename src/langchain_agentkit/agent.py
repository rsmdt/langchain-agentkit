...
def _validate_handler_signature(handler: Any, class_name: str) -> tuple[set[str], type]:
    ...
    if not params:
        raise ValueError(
            f"class {class_name}(agent): handler must accept at least "
    ...
    if first.kind not in (
        ...
        raise ValueError(
            f"class {class_name}(agent): handler's first parameter must be "
    ...
    for param in params[1:]:
        ...
        if param.kind == inspect.Parameter.KEYWORD_ONLY:
            if param.name not in _INJECTABLE_PARAMS:
                raise ValueError(
                    f"class {class_name}(agent): unknown handler parameter "
            ...
            raise ValueError(
                f"class {class_name}(agent): handler parameter '{param.name}' "
