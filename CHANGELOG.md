...
### Added
- **Bulk file transfer for `DaytonaBackend`.** `upload_files` / `download_files` route through the Daytona SDK's multipart endpoints — one HTTP request for N files instead of N base64-through-shell calls. Roughly 4× faster on real Daytona instances and no longer ARG_MAX-bound, so large or binary files round-trip correctly.
