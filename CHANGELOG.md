...
- Backend conformance is now opt-in per run, e.g. `uv run --extra dev --extra mirage poe test:mirage` (likewise `test:agentfs`, `test:daytona`, `test:bubblewrap`). The in-process backends run as a concurrent CI matrix: `agentfs` and `mirage` need no external dependencies; `daytona` is credential-gated and skips with a notice when `DAYTONA_API_KEY` isn't configured. `bubblewrap` runs in a dedicated CI job inside a privileged Docker container (built from `tests/docker/Dockerfile`) — `bwrap` needs privileged namespace operations the bare runner denies, and the privilege applies only to the outer container, leaving the inner sandbox's own isolation intact. This also stops the daytona suite from erroring in the default integration matrix when stray credentials are present.
