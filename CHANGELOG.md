...
- Backend conformance is now opt-in per run, e.g. `uv run --extra dev --extra mirage poe test:mirage` (likewise `test:agentfs`, `test:daytona`, `test:bubblewrap`). The in-process backends run as a concurrent CI matrix: `agentfs` and `mirage` need no external dependencies; `daytona` is credential-gated and skips with a notice when `DAYTONA_API_KEY` isn't configured. `bubblewrap` is excluded from CI — `bwrap` requires an unprivileged network namespace the bare runner forbids — and is verified locally via `tests/docker/Dockerfile`. This also stops the daytona suite from erroring in the default integration matrix when stray credentials are present.
