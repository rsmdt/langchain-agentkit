...
- Backend conformance is now opt-in per run, e.g. `uv run --extra dev --extra mirage poe test:mirage` (likewise `test:agentfs`, `test:daytona`, `test:bubblewrap`). The in-process backends run as a concurrent CI matrix: `agentfs` and `mirage` need no external dependencies; `daytona` is credential-gated and skips with a notice when `DAYTONA_API_KEY` isn't configured. `bubblewrap` runs as a dedicated leg of the same matrix, inside a privileged Docker container (built from `tests/docker/Dockerfile`) — `bwrap` needs privileged namespace operations the bare runner denies, and the privilege applies only to the outer container, leaving the inner sandbox's own isolation intact. This also stops the daytona suite from erroring in the default integration matrix when stray credentials are present.
- CI test coverage now spans unit, integration, and eval-dataset validation on every push: the per-Python `Tests` matrix runs `tests/unit/`, the non-backend `tests/integration/` suite, and `tests/evals/test_eval_runner.py`. The backend conformance lives in the `backends` matrix and the LLM evals (requiring `OPENAI_API_KEY`) in the gated `evals` job.
...
### Added
- **Bulk file transfer for `DaytonaBackend`.** `upload` / `download` route through the Daytona SDK's multipart endpoints — one HTTP request for N files instead of N base64-through-shell calls. Roughly 4× faster on real Daytona instances and no longer ARG_MAX-bound, so large or binary files round-trip correctly.
- **Sandbox environment introspection.** `SandboxBackend.environment()` returns a `SandboxEnvironment` snapshot (`os`, `shell`, `cwd`, plus `available_tools` from a curated probe of `git`/`rg`/`jq`); `FilesystemExtension` probes once during async setup and renders the result as an `<env>` block in the system prompt, matching Claude Code's convention. `OSBackend` populates instantly via `platform.*` / `shutil.which`; `DaytonaBackend` issues a single shell probe (~30-50 ms) and caches for the session.
- **`[daytona]` install extra.** `pip install langchain-agentkit[daytona]` makes the optional `daytona-sdk` dependency explicit at install time.

### Changed
- **BREAKING**: **Result-typed errors replace exceptions across the backend protocol.** `read`, `read_bytes`, `write`, `edit`, `upload`, and `download` now return result dataclasses (`ReadResult`, `WriteResult`, `EditResult`, `FileUploadResult`, `FileDownloadResult`) with stable `Literal` error codes (`file_not_found`, `is_directory`, `permission_denied`, `decode_error`, `io_error`, `old_string_not_found`, `ambiguous_match`) instead of raising. Tool-layer code branches on `result.error` to produce code-aware LLM messages without `try/except` chains. Truly unexpected failures (network, SDK bugs) still raise.
- **BREAKING**: **Backend protocol split into three capability tiers.** `BackendProtocol` (file ops), `SandboxBackend` (adds `execute` and `environment`), and `FileTransferBackend` (adds `upload`/`download`) replace the previous single mandatory surface. Backends declare which tiers they implement; consumers gate behavior on `isinstance(backend, SandboxBackend)` so e.g. the bash tool registers only when the backend supports shell execution.
- **BREAKING**: **Concrete backends require explicit submodule imports.** `OSBackend` and `DaytonaBackend` are no longer re-exported from `langchain_agentkit.backends` or the package root — import each from its own submodule (`langchain_agentkit.backends.os`, `langchain_agentkit.backends.daytona`). Optional-dependency gates surface at the import line: importing the daytona module fails with a clear `ImportError` if `daytona-sdk` is missing, rather than blowing up deep inside a method call.
- `DaytonaBackend` SDK imports moved to module top with `TYPE_CHECKING` for the `Sandbox` type, dropping the lazy in-method imports.
