...
class TestReadTool:
    ...
    def test_truncated_read_returns_partial_content(self):
        lines = "\n".join(f"line {i}" for i in range(100))
        backend, _ = _make_backend_with_files({"/big.txt": lines})
        tool = create_filesystem_tools(backend)[0]

        result = tool.invoke({"file_path": "/big.txt", "limit": 10})
    ...
    def test_small_file_returns_all_content(self):
        backend, _ = _make_backend_with_files({"/small.txt": "a\nb\nc\n"})
        tool = create_filesystem_tools(backend)[0]

        result = tool.invoke({"file_path": "/small.txt"})
    ...
    def test_reads_pdf_with_metadata_message(self):
        backend, tmpdir = _make_backend_with_files({})
...
class TestWriteTool:
    ...
    def test_updates_existing_file(self):
        ...
        result = tool.invoke({"file_path": "/f.txt", "content": "new"})
    ...
    def test_creates_nested_path(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = tool.invoke({"file_path": "/a/b/c/deep.txt", "content": "deep"})
...
class TestEditTool:
    ...
    def test_empty_old_string_creates_file(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            backend = OSBackend(tmpdir)
            tool = create_filesystem_tools(backend)[2]
    ...
    def test_empty_old_string_fills_empty_file(self):
        backend, tmpdir = _make_backend_with_files({"/empty.txt": ""})
        tool = create_filesystem_tools(backend)[2]
    ...
    def test_empty_old_string_rejects_nonempty_file(self):
        backend, tmpdir = _make_backend_with_files({"/f.txt": "existing"})
        tool = create_filesystem_tools(backend)[2]
    ...
    def test_delete_strips_trailing_newline(self):
        backend, tmpdir = _make_backend_with_files({"/f.txt": "line1\nline2\nline3\n"})
        tool = create_filesystem_tools(backend)[2]
    ...
    def test_strips_trailing_whitespace_from_new_string(self):
        backend, tmpdir = _make_backend_with_files({"/f.py": "x = 1\n"})
        tool = create_filesystem_tools(backend)[2]
    ...
    def test_preserves_trailing_whitespace_in_markdown(self):
        backend, tmpdir = _make_backend_with_files({"/f.md": "line one\n"})
        tool = create_filesystem_tools(backend)[2]
    ...
    def test_delete_preserves_when_old_string_ends_with_newline(self):
        backend, tmpdir = _make_backend_with_files({"/f.txt": "line1\nline2\nline3\n"})
        tool = create_filesystem_tools(backend)[2]
...
class TestGrepTool:
    ...
    def test_bash_tool_accepts_description(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = tool.invoke({"command": "echo hi", "description": "Print greeting"})
...
class TestStructuredOutput:
    ...
    def _call_raw(self, tool, input_dict):
        """Call the inner function directly to get (content, artifact) tuple."""
        # Access the underlying function, bypassing StructuredTool wrapper
        func = tool.func
        return func(**input_dict)

    def test_read_text_artifact(self):
        backend, _ = _make_backend_with_files({"/f.txt": "hello\nworld\n"})
        tool = create_filesystem_tools(backend)[0]
        content, artifact = self._call_raw(tool, {"file_path": "/f.txt"})
    ...
    def test_read_image_artifact(self):
        backend, _ = _make_backend_with_files({})
        backend.write("/img.png", b"\x89PNG\r\n\x1a\n")
        tool = create_filesystem_tools(backend)[0]
        content, artifact = self._call_raw(tool, {"file_path": "/img.png"})
    ...
    def test_read_pdf_artifact(self):
        backend, _ = _make_backend_with_files({})
        backend.write("/doc.pdf", b"%PDF-1.4")
        tool = create_filesystem_tools(backend)[0]
        content, artifact = self._call_raw(tool, {"file_path": "/doc.pdf"})
    ...
    def test_read_notebook_artifact(self):
        ...
        tool = create_filesystem_tools(backend)[0]
        content, artifact = self._call_raw(tool, {"file_path": "/nb.ipynb"})

        assert artifact["type"] == "notebook"
        assert isinstance(artifact["cells"], list)
        assert artifact["cells"][0]["source"] == "x = 1"

    def test_write_create_artifact(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            backend = OSBackend(tmpdir)
            tool = create_filesystem_tools(backend)[1]
            content, artifact = self._call_raw(tool, {"file_path": "/new.txt", "content": "hi"})
    ...
    def test_write_update_artifact(self):
        backend, _ = _make_backend_with_files({"/f.txt": "old"})
        tool = create_filesystem_tools(backend)[1]
        content, artifact = self._call_raw(tool, {"file_path": "/f.txt", "content": "new"})
    ...
    def test_glob_artifact(self):
        backend, _ = _make_backend_with_files({"/a.txt": "", "/b.txt": ""})
        tool = create_filesystem_tools(backend)[3]
        content, artifact = self._call_raw(tool, {"pattern": "**/*.txt"})
    ...
    def test_grep_files_artifact(self):
        backend, _ = _make_backend_with_files({"/f.txt": "match here"})
        tool = create_filesystem_tools(backend)[4]
        content, artifact = self._call_raw(tool, {"pattern": "match"})

        assert artifact["mode"] == "files_with_matches"
        assert artifact["numFiles"] == 1
        assert "/f.txt" in artifact["filenames"]

    def test_grep_content_artifact(self):
    ...
    def test_grep_count_artifact(self):
    ...
    def test_bash_artifact(self):
        from langchain_agentkit.extensions.filesystem.extension import _build_bash_tool

        with tempfile.TemporaryDirectory() as tmpdir:
            backend = OSBackend(tmpdir)
            tool = _build_bash_tool(backend)
            content, artifact = tool.func(command="echo hello")
    ...
    def test_write_update_has_structured_patch(self):
        backend, _ = _make_backend_with_files({"/f.txt": "hello\nworld\n"})
        tool = create_filesystem_tools(backend)[1]
        _, artifact = self._call_raw(tool, {"file_path": "/f.txt", "content": "hello\nplanet\n"})
    ...
    def test_write_create_has_empty_patch(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            backend = OSBackend(tmpdir)
            tool = create_filesystem_tools(backend)[1]
            _, artifact = self._call_raw(tool, {"file_path": "/new.txt", "content": "hi"})

            assert artifact["structuredPatch"] == []

    def test_edit_has_structured_patch(self):
        backend, _ = _make_backend_with_files({"/f.txt": "hello world"})
        tool = create_filesystem_tools(backend)[2]
    ...
    def test_read_text_file_unchanged_on_second_read(self):
        backend, _ = _make_backend_with_files({"/f.txt": "hello\n"})
        tool = create_filesystem_tools(backend)[0]
        # First read populates cache
        content1, art1 = self._call_raw(tool, {"file_path": "/f.txt"})
        assert art1["type"] == "text"
        # Second read with same params returns file_unchanged
        content2, art2 = self._call_raw(tool, {"file_path": "/f.txt"})
        assert art2["type"] == "file_unchanged"
        assert "not changed" in content2.lower()

    def test_read_image_has_multimodal_content(self):
        backend, _ = _make_backend_with_files({})
        backend.write("/img.png", b"\x89PNG\r\n\x1a\n")
        tool = create_filesystem_tools(backend)[0]
        content, _ = self._call_raw(tool, {"file_path": "/img.png"})
    ...
    def test_read_pdf_message_format(self):
        backend, _ = _make_backend_with_files({})
        backend.write("/doc.pdf", b"%PDF-1.4")
        tool = create_filesystem_tools(backend)[0]
        content, _ = self._call_raw(tool, {"file_path": "/doc.pdf"})
        assert content.startswith("PDF file read:")
        assert "/doc.pdf" in content

    def test_read_empty_file_warning(self):
        backend, _ = _make_backend_with_files({"/empty.txt": ""})
        tool = create_filesystem_tools(backend)[0]
        content, artifact = self._call_raw(tool, {"file_path": "/empty.txt"})
        assert "empty" in content.lower()
        assert artifact["totalLines"] == 0

    def test_grep_files_with_matches_shows_found_count(self):
        backend, _ = _make_backend_with_files({"/a.txt": "x", "/b.txt": "x"})
        tool = create_filesystem_tools(backend)[4]
        result = tool.invoke({"pattern": "x"})

        assert "Found 2 file(s)" in result

    def test_grep_truncated_shows_pagination(self):
        files = {f"/{i}.txt": "match" for i in range(300)}
        backend, _ = _make_backend_with_files(files)
        tool = create_filesystem_tools(backend)[4]
